...
fun File.deleteDir(): Boolean {
    if (this.isDirectory) {
        val chaildren = this.list()
        chaildren?.forEachIndexed { index, s ->
            val secc = File(this, chaildren[index]).deleteDir()
            if (!secc) {
                return false
            }
        }
...
fun File.getFolderSize():Long{
    ...
    try {
        ...
        fileList?.forEachIndexed { index, file ->
            size += if (fileList[index]?.isDirectory == true){
                fileList[index]?.getFolderSize() ?: 0
            }else{
