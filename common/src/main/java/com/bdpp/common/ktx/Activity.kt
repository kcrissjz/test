...
fun startKtxActivity(intent: Intent) = topActivity.startActivity(intent)

inline fun <reified T : Activity> startKtxActivity(
  vararg pairs: Pair<String, Any?>,
  crossinline block: Intent.() -> Unit = {}
) =
  topActivity.startKtxActivity<T>(pairs = pairs, block = block)
...
fun ComponentActivity.pressBackTwiceToExitApp(toastText: String, delayMillis: Long = 2000, owner: LifecycleOwner = this,action: () -> Unit) =
  pressBackTwiceToExitApp(delayMillis, owner,{ toast(toastText) },{action()})

fun ComponentActivity.pressBackTwiceToExitApp(@StringRes toastText: Int, delayMillis: Long = 2000, owner: LifecycleOwner = this,action: () -> Unit) =
  pressBackTwiceToExitApp(delayMillis, owner,{ toast(toastText) },{action()})
